$(document).ready(function () {
	function hex_initial_animation() {
		...
		$(".hoverblock").velocity("fadeOut", { delay: 0, duration: 0 });
	}
	...
	// Muestra el producto M despues de cargada la pagina.
	function primer(){
		$x='#hex-init';
		var title_color = $($x).parent().attr("data-color");
		var title_name = $($x).parent().attr("data-title");
		var desc_name = $($x).parent().attr("data-content");
		function hex_description() {
			$('.code-description').velocity('stop', true).velocity("transition.slideRightBigIn");
			$('.' + desc_name).siblings().removeClass('desc-active');
			setTimeout(function () {
		...
		hex_description();
		$($x).parent().addClass('hexactive');
		$('.hexactive').velocity({ }, { duration: 200 });
	}
});
